...
# 0: 일반 조건검색
# 1: 실시간 조건검색
CONDITION_SEARCH_TYPE = get_nested(SETTINGS, "condition.search_type", 0)

CONDITION_WATCH_SECONDS = get_nested(SETTINGS, "condition.watch_seconds", 1)
...
MARKET_OPEN_TIME = get_nested(
    SETTINGS,
    "trading_loop.market_open_time",
    "09:00",
)

MARKET_CLOSE_TIME = get_nested(
    SETTINGS,
    "trading_loop.market_close_time",
    "15:30",
)
...
MAX_LOOP_COUNT = get_nested(
    SETTINGS,
    "trading_loop.max_loop_count",
    3,
)
...
# v8.2:
# kiwoom: 키움 HTS 조건검색 사용
# code: 코드/YAML 기반 조건검색 사용
# both: 둘 다 사용
LOOP_CONDITION_SOURCE = get_nested(
    SETTINGS,
    "trading_loop.condition_source",
    "code",
)
...
NOTIFY_ON_INFO = get_nested(
    SETTINGS,
    "notification.notify_on_info",
    True,
)

NOTIFY_ON_ORDER = get_nested(
    SETTINGS,
    "notification.notify_on_order",
    True,
)

NOTIFY_ON_ERROR = get_nested(
    SETTINGS,
    "notification.notify_on_error",
    True,
)
